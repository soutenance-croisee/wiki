...
<body>
    <div class="p-2 md:p-12">
                    ...
                    <?php foreach ($data['wikis'] as $wiki): ?>
                        <?php if ($wiki['category_id'] == $category['id']): ?>
                            <div
                                ...
                                <!-- Update Button -->
                                <button class="absolute top-2 left-2 text-blue-500 hover:text-blue-700"
                                    onclick="openEditWikiModal('<?= $wiki['id']; ?>')">Update</button>
        ...
        <div id="EditWiki" class="hidden fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
            <!-- ... (your existing edit wiki modal) ... -->
        </div>
    </div>

    <script>
        function confirmDeleteWiki(wikiId) {
            // Implement your delete logic here, for example, show a confirmation modal
            console.log("Deleting wiki with ID: " + wikiId);
        }

        function openEditWikiModal(wikiId) {
            // Implement your logic to open the edit wiki modal here
            console.log("Opening edit modal for wiki with ID: " + wikiId);
        }
    </script>
</body>
